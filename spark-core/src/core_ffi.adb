...
package body Core_FFI is
   ...
   begin
      ...
      if Input_Str'Length = 0 or else Input_Str'Length > Core.Max_URL_Length then
         Output_Len.all := 0;
      ...
      if Result.Status = Success then
         declare
            ...
            C_Array : char_array := To_C (Output_Str, Append_Nul => True);
         begin
   ...
   begin
      ...
      Key := Core.Secret_Key (Secret_Str (Secret_Str'First .. Secret_Str'First + 31));
      ...
      -- Generate short code
      declare
         Code : constant Short_Code := Make_Short_Code (Canon_Result.URL, Key);
         Code_Str : constant String := To_String (Code);
         C_Array : char_array := To_C (Code_Str, Append_Nul => True);
      begin
         -- Copy to output buffer
         Update (Output, 0, C_Array, Check => True);
      end;
