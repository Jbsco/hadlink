...
class Store m where
  ...
  put :: ShortCode -> ValidURL -> m ()
  
  -- | Lookup a URL by short code
  get :: ShortCode -> m (Maybe ValidURL)
  
  -- | Check if a short code exists
  exists :: ShortCode -> m Bool
...
instance Store (SQLiteStore -> IO) where
  put shortCode url store@(SQLiteStore conn) = do
  ...
  get shortCode store@(SQLiteStore conn) = do
    let ShortCode code = shortCode
  ...
  exists shortCode store@(SQLiteStore conn) = do
    let ShortCode code = shortCode
    results <- SQL.query conn
