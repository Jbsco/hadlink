...
URL shortening is a deceptively small problem with disproportionate blast radius. Any service that accepts arbitrary URLs and redirects traffic to them becomes, in effect, an open relay. Shorteners are frequent abuse targets for spam campaigns, phishing attacks, and malware distribution. The opacity of the short code, hiding the destination until the redirect occurs, is precisely what makes them useful and precisely what makes them dangerous.
...
The SPARK core contains no I/O, networking, storage, concurrency, or configuration parsing. It is pure computation with explicit preconditions and postconditions. All 137 proof obligations are verified automatically by the CVC5 SMT solver.

\subsection{Haskell Service Layer}

The service layer composes the system from conservative dependencies. HTTP handling uses Warp, chosen for its minimal API surface and proven reliability. Storage uses SQLite in WAL mode. Rate limiting, structured logging, and proof-of-work validation are implemented in Haskell and tested via Hedgehog property tests.
...
Two separate executables serve different trust profiles. \texttt{hadlink-redirect} handles the read path: it looks up short codes and returns redirects. It is fast, stateless, and links no SPARK code; the redirect binary does not depend on \texttt{libHadlink\_Core.so}. It is designed to be exposed to the public network.
...
SQLite in WAL mode provides the storage layer. The schema is minimal: short code (primary key), canonical URL, and creation timestamp. The write path is append-only; there is no update or delete operation.
...
The SPARK core provides machine-checked guarantees about the system's behavior. All stored URLs have a valid HTTP or HTTPS scheme. No stored URL contains embedded credentials. No stored URL points to a private address. Short codes are exactly 8 Base62 characters. All array accesses are proven in-bounds, and the encoding logic is free of integer overflow.
...
hadlink supports three deployment methods. Docker images use pre-built binaries from GitHub Releases or can be built from source. Systemd unit files provide direct installation with security hardening. An Arch Linux AUR package (\texttt{hadlink-bin}) offers distribution-native installation.
...
Configuration follows a minimalist approach: environment variables only, with no configuration file parser. There is no YAML, no TOML, no JSON configuration. Secret management uses systemd's \texttt{EnvironmentFile} directive, keeping secrets out of unit files and command lines.
