...
Every precondition, postcondition, and type declaration is a decision about what the system can and cannot safely assume. Consider the contract for \texttt{hadlink}'s canonicalization function:
...
SPARK constraints directly influenced \texttt{hadlink}'s architecture in several ways. Each constraint, initially encountered as a limitation, became an architectural decision that improved the system.
...
An attempt to use \texttt{Type\_Invariant} was blocked by SPARK RM 7.3.2(2) \cite{sparkrm}, which restricts type invariants in certain contexts. The workaround, a ghost lemma with two documented assumes for pure function determinism, confined all trust assumptions to a single location:
...
\bottomrule
\end{tabular}
\caption{Verification summary for \texttt{hadlink}'s SPARK core.}
\end{table}
...
\bibitem{sparkrm}
AdaCore.
\textit{SPARK Reference Manual}.
\url{https://docs.adacore.com/spark2014-docs/html/lrm/}
